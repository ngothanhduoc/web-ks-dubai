...
<div id="contentwrapper" class="contentwrapper lineheight21">
    <form class="stdform stdform2" id="frm-add-article" role="form" action="" method="POST" enctype="multipart/form-data">
        ...
        <p>
            ...
            <span class="field">
                <select name='id_group_product'>
                    <?php
                    foreach ($group_product as $key => $value) {
                        if ($value['name'] == @$group_name)
                            echo "<option selected = 'selected' value='" . $value['id_group_product'] . "'>" . $value['name'] . "</option>";
                        else
                            echo "<option value='" . $value['id_group_product'] . "'>" . $value['name'] . "</option>";
                    }
                    ?>
                </select>
            </span>
