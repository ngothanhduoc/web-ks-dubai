        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if($this->form_validation->run() == TRUE){
                ...
                if(empty($Id) === FALSE && is_numeric($Id)){
                    ...
                    if(empty($arrParam['sub_image']) === TRUE) $arrParam['sub_image'] = '/frontend/assets/images/no-image-game.png'; 
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if($this->form_validation->run() == TRUE){
                ...
                if(empty($Id) === FALSE && is_numeric($Id)){
                   ...
                   if(empty($arrParam['menu_bg']) === TRUE) $arrParam['menu_bg'] = '/frontend/assets/wap/image/detail-menu-bg.png';
                    $this->m_backend->jqxUpdate('game','id_game',$Id,$arrParam);
                    $response['code'] = 0;
                }
